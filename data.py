...
class BSDS300_images(VisionDataset):
    ...
    def __init__(self, root, sigma, train=True, transform=None,noise="Gaussian", download=False):
    ...
    def __getitem__(self, idx):
        ...
        if(self.noise == "Poisson"):
            ...
            im_noisy = np.random.poisson(im * vals) / float(vals)
            im_noisy = torch.from_numpy(im_noisy).float().cuda()
        else :
...
class BerkeleyLoader(data.DataLoader):
    def __init__(self, sigma, train=True,noise="Gaussian", **kwargs):
        if train:
        ...
        dataset = BSDS300_images("data", sigma, train=train, transform=transform,noise=noise, download=True)
        super(BerkeleyLoader, self).__init__(dataset, **kwargs)


if __name__ == "__main__":
    train_data = BerkeleyLoader(25, train=True, noise="Gaussian",batch_size=10)
