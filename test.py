...
def parse_args():
    ...
    parser.add_argument('--noise',choices=['Gaussian', 'Poisson'], default='Gaussian', help='Type of noise to use for testing')
...
def test(model,noise):
    model.eval()
    for sigma in [15, 25]:
        print("\nSigma =", sigma)
        test_loader = BerkeleyLoader(sigma=sigma, train=False, noise=noise, num_workers=1)
